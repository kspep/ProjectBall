...
class Bacterium {
    constructor(team, x, y, generation, mutations = []) {
        ...
        this.nextAllyDecisionTime = Date.now() + 4000 + Math.random() * 3000;
        this.nextEnemyDecisionTime = 0;
    ...
    think() {
        ...
        // Обработка врагов сразу при попадании в зону обзора (тип новая функция, тест)
        for (const enemy of bacteria) {
            if (enemy === this || enemy.team === this.team) continue;
            const dx = this.x - enemy.x;
            const dy = this.y - enemy.y;
            const distSq = dx * dx + dy * dy;
            if (distSq <= this.visionRadius * this.visionRadius) {
                if (now >= this.nextEnemyDecisionTime) {
                    this.nextEnemyDecisionTime = now + 1000 + Math.random() * 2000;
        ...
        if (now < this.nextAllyDecisionTime) return;
        this.nextAllyDecisionTime = now + 4000 + Math.random() * 3000;

        const nearby = [];
        for (let i = 0; i < bacteria.length; i++) {
            const other = bacteria[i];
            if (other !== this) {
                const dx = this.x - other.x;
                const dy = this.y - other.y;
                const dist = dx * dx + dy * dy;
                if (dist <= this.visionRadius * this.visionRadius) {
                    nearby.push(other);
                }
            }
        }
        ...
        for (const b of nearby) {
            if (b.team === this.team) friends.push(b);
            else enemies.push(b);
        }
